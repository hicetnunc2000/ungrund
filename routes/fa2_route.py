...
class publish_fa2(Resource):
    def get(self):
        try:
            ...
            op = pytezos.origination(script=contract.script(storage={ "administrator" : pytezos.key.public_key_hash(), "all_tokens" : 0, "ledger" : {}, "paused" : False, "tokens" : {}, "totalSupply" : 0  })).autofill().sign().inject(_async=False, num_blocks_wait=2)
